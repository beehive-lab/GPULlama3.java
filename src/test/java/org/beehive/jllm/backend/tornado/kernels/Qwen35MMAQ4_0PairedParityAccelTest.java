...
// @formatter:off
/**
 * The paired-load {@code Q4_0} tensor-core projection against the byte-load kernel it was copied
 * from, on the device, over identical input bytes.
 *
 ...
 */
// @formatter:on
public class Qwen35MMAQ4_0PairedParityAccelTest {
    ...
    private static void assertBitParity(String what, int m, int n, int k, byte[] raw, long seed)
        ...
        scheduler.addWorkerGrid("parity.paired", grid(m, n, Qwen35MMAKernels.BN));
    ...
    /** One K block, and many: the pair index never leaves its own block. */
    @Test
    public void everyBlockCountAgreesBitForBit() throws Exception {
        for (int k : new int[] {32, 64, 1024, 5120}) {
