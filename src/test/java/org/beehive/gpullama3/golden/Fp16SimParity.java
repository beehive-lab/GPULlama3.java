...
public final class Fp16SimParity {
     ...
     */
    private static List<float[]> cpuRows(Path path, boolean simulateFp16, List<Integer> forced) throws Exception {
        ...
        tokens.addAll(cf.encodeMessage(new ChatFormat.Message(ChatFormat.Role.USER, GoldenCapture.PROMPT)));
        tokens.addAll(cf.encodeHeader(new ChatFormat.Message(ChatFormat.Role.ASSISTANT, "")));

        // Mirrors InferenceEngine.generateTokens*: the first forward is of state.latestToken, each
        // prompt token is fed on the following step, and the first captured row is the forward that
        // happens once the prompt is exhausted. Getting this shifted by one compares different
    ...
    /**
     * One decode step. Mirrors {@link InferenceCore#forwardGranite} (which is the Llama forward plus
     * the µP multipliers; for a non-Granite model the multipliers are the identity), with optional
     * FP16 rounding where the GPU stores FP16.
     */
    private static float[] forward(Model model, State state, int token, int position, boolean simulateFp16) {
        ...
        float attentionScale = granite ? ((GraniteConfiguration) config).attentionScale()
        ...
        for (int l = 0; l < config.numberOfLayers(); l++) {
            InferenceCore.rmsnorm(state.xb, state.x, w.rms_att_weight[l], 0, dim, config.rmsNormEps());
            if (simulateFp16) {
            ...
            for (int h = 0; h < config.numberOfHeads(); h++) {
                ...
                for (int t = 0; t <= position; t++) {
                    ...
                    float score = state.q.dot(qOffset, state.keyCache[l], keyOffset, headSize) * attentionScale;
            ...
            InferenceCore.rmsnorm(state.xb, state.x, w.rms_ffn_weight[l], 0, dim, config.rmsNormEps());
    ...
    private static void compare(String label, List<float[]> a, List<float[]> b) {
        ...
        System.out.printf("  %-34s worst relL2=%.4g  maxAbs=%.4g  argmax diffs=%d/%d%n",
