 ...
 *
 * <p>Sampling is greedy argmax, so temperature, top-p and seed are fixed and have no effect —
 * the only varying inputs are the prompt and the context length, held identical across configs.
 */
public final class LogitDump {
    ...
    public static void main(String[] args) throws Exception {
        ...
        prompt.addAll(cf.encodeMessage(new ChatFormat.Message(ChatFormat.Role.USER,
                GoldenCapture.PROMPT)));
        ...
        try {
            if (gpu) {
                ...
                m.generateTokensGPU(state, 0, prompt, Set.of(), prompt.size() + positions,
                        capture, false, null, plan);
            } else {
                m.generateTokens(state, 0, prompt, Set.of(), prompt.size() + positions,
                        capture, false, null);
            }
        } finally {
            if (plan != null) {
