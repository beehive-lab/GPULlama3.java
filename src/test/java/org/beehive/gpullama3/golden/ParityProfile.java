...
/**
 * Diagnostic: the CPU↔GPU logit distributions the T1.5 thresholds are derived from.
 *
 * <p>Reports, per configuration, the quantities the parity gate asserts on: elementwise violations
 * of {@code atol + rtol·|ref|}, relative L2, cosine similarity, the error percentiles, and the
...
public final class ParityProfile {
    ...
    private static void profile(String label, Path model) throws Exception {
        ...
        System.out.printf("  elementwise (atol=%.0e rtol=%.0e): violations=%d/%d (%.4g%%) maxRatio=%.4g%n",
                ATOL, RTOL, violations, total, 100.0 * violations / total, maxRatio);
        System.out.printf("  absErr: p50=%.3g p99=%.3g p99.99=%.3g max=%.3g%n",
                allAbs[(int) (total * 0.50)], allAbs[(int) (total * 0.99)],
                allAbs[(int) (total * 0.9999)], maxAbs);
        System.out.printf("  worst relL2=%.4g (row %d)  minCosine=%.8f%n", worstRelL2, worstRelL2Row, minCos);
        System.out.printf("  argmax disagreements=%d/%d  top5 overlap=%.3f/5  top10 overlap=%.3f/10%n",
                argmaxDisagreements, rows, top5Overlap / (double) rows, top10Overlap / (double) rows);
        ...
        for (double atol : new double[] { 1e-4, 1e-3, 5e-3, 1e-2, 2e-2, 5e-2 }) {
            ...
            for (int r = 0; r < rows; r++) {
                ...
                float[] got = gpu.rows.get(r);
                for (int i = 0; i < ref.length; i++) {
                    if (Math.abs((double) ref[i] - got[i]) > atol + RTOL * Math.abs((double) ref[i])) {
                        bad++;
                    }
                }
            }
            System.out.printf("    atol=%.0e rtol=%.0e -> violations=%d (%.4g%%)%n",
