...
public final class Fp16DeterminismProbe {
    ...
    private static final String PROMPT = "Explain what a matrix multiplication is in one paragraph.";
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("probe model=" + model.getFileName() + " runs=" + runs + " gpu=" + gpu
        ...
        for (int r = 0; r < runs; r++) {
            ...
            try {
                if (gpu) {
                    ...
                    m.generateTokensGPU(state, 0, prompt, Set.of(),
                            prompt.size() + 1 + extraTokens, capture, false, null, plan);
                } else {
                    m.generateTokens(state, 0, prompt, Set.of(),
                            prompt.size() + 1 + extraTokens, capture, false, null);
                }
                ...
                wrapX.add(snapshot(state.wrapX));
                tempLogits.add(snapshot(state.tempLogits));
                xbFp16.add(snapshotHalf(state.wrapXbFP16));
            } finally {
                if (plan != null) {
        ...
        System.out.println("KV read-back check: K nonZero=" + kNonZero + " V nonZero=" + vNonZero
                + (kNonZero == 0 ? "  <-- K NOT READ BACK, layer result inconclusive" : ""));

        int kvPerLayer = keys.get(0).length / layers;
        System.out.println("layers=" + layers + " kvCacheLen=" + keys.get(0).length
                + " perLayer=" + kvPerLayer);

        System.out.println("\n=== token sequences identical across runs: "
                + tokens.stream().distinct().count() + " distinct ===");
    ...
    private static void reportEarliestLayer(String tag, List<float[]> snaps, int layers, int perLayer) {
        for (int layer = 0; layer < layers; layer++) {
            ...
            if (differing > 0) {
                System.out.printf("  %s: earliest divergent layer = %d  (differing=%d, maxAbs=%.6g)%n",
    ...
    private static void reportBuffer(String label, List<float[]> snaps) {
        ...
        System.out.printf("  %s len=%d nonZero=%d changed=%d maxAbs=%.6g %s%n",
                label, a0.length, nonZero, changed, maxAbs,
                nonZero == 0 ? "NOT READ BACK (inconclusive)"
                        : (changed == 0 ? "IDENTICAL" : "*** DIVERGES ***"));
    }

    private static float[] snapshotHalf(uk.ac.manchester.tornado.api.types.arrays.HalfFloatArray a) {
        if (a == null) {
            return null;
        }
    ...
    private static void stats(String label, float[] a, float[] b) {
        ...
        int argA = argmax(a);
        int argB = argmax(b);
        System.out.printf("  %s: changed=%d/%d maxAbs=%.6g meanAbs=%.4g maxRel=%.4g argmax %d->%d %s topk5=%s topk10=%s%n",
                label, diff, a.length, maxAbs, sum / a.length, maxRel, argA, argB,
