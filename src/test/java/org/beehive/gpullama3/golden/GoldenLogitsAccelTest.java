...
/**
 * T1.4 — golden logits, Class B.
 *
 * <p>Runs only under {@code -Paccel-tests}; excluded from {@code mvn test}, which must never need a
 ...
 * token id must match, and the retained final row must be identical under
 * {@code Float.floatToRawIntBits}. NaN or Inf anywhere fails immediately, before comparison — a
 * NaN-vs-NaN "match" must not pass.
 */
public class GoldenLogitsAccelTest {
    ...
    private void assertGolden(Fixture fixture) throws Exception {
        ...
        if (!Files.isDirectory(goldenDir)) {
            System.out.println("[SKIP] environment absent — no committed golden at " + goldenDir
        ...
        if (!pinnedTuple) {
            System.out.println("[WARN] not the pinned tuple (" + expected.metadata.get("device_name")
        ...
        if (pinnedTuple && bitExact) {
            ...
            for (int r = 0; r < actual.rows.size(); r++) {
                assertEquals("logits row " + r + " differs from golden",
                        expectedHashes.get(r), GoldenRecord.hashRow(actual.rows.get(r)));
            }
            ...
            for (int i = 0; i < finalRow.length; i++) {
                assertEquals("final row element " + i + " differs bitwise",
    ...
    public void goldensDeclareTheirExecutionFlags() throws Exception {
        ...
        assertEquals("goldens must be recorded with bailout disabled (C4)",
                "false", g.metadata.get("recover_bailout"));
        assertEquals("goldens must be recorded with host-side sampling",
                "false", g.metadata.get("device_sample"));
