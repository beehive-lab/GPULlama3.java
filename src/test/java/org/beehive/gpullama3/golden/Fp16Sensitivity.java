...
public final class Fp16Sensitivity {
    ...
    private static void report(Path modelPath) throws Exception {
        ...
        System.out.printf("%-34s embScale=%-5.3g  fp16 round-trip relL2: mean=%.3e worst=%.3e"
                        + "  |xb| in [%.3g, %.3g]  overflow=%d subnormal=%d%n",
                modelPath.getFileName(), embeddingScale, sumRel / tokens.length, worstRel,
                minNonZeroAbsX, maxAbsX, overflow, subnormal);
    ...
    private static void denormals(String label, FloatTensor t, int count) {
        ...
        System.out.printf("%-14s denormal-FP16 weights: %d/%d (%.3f%%)  their share of |w|: %.3g%%%n",
                label, denormal, count, 100.0 * denormal / count,
