...
/**
 * Diagnostic for T1.5: localizes the CPU↔GPU logit gap to a specific layer-0 operation.
 *
 * <p>The full-model parity test can only say the paths disagree by ~3.5 at the logits. This
 * compares, at layer 0 of the first generated position, each intermediate the GPU exposes under
 * {@code -Dgpullama3.diag.transfers} against the same quantity recomputed on the CPU:
 *
 * <ol>
 *   <li>the RMS scale ({@code state.temp[0]}) — a pure FP32 reduction, so a difference here is
 *       accumulation order;</li>
 ...
 *       format is the whole difference;</li>
 ...
 *       its input: if it matches, the projection is faithful and the error was inherited.</li>
 * </ol>
 *
...
public final class LayerParity {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            gpuModel.generateTokensGPU(gpuState, 0, prompt, Set.of(), prompt.size() + 1, greedy, false, null, plan);
            token = sink.get(0);
            InferenceCore.forwardTornadoVM(gpuModel, gpuState, token, prompt.size(), plan);
            gpuTemp = snap(gpuState.temp);
            gpuXb = snapHalf(gpuState.wrapXbFP16);
            gpuQ = snap(gpuState.wrapQ);
        } finally {
            plan.freeTornadoExecutionPlan();
        }

        // ---- CPU: the same layer-0 quantities, recomputed from the standard weights.
        Model cpuModel = ModelLoader.loadModel(modelPath, GoldenCapture.CONTEXT_LENGTH, true, false);
        ...
        // wrapQ is read back at the end of the layer graph, i.e. after rope_and_kv_cache has rotated
        // it in place, so the CPU side has to be rotated too before the two are comparable.
        ...
        System.out.printf("token=%d position=%d dim=%d qDim=%d headSize=%d ropeTheta=%s%n",
                token, position, dim, qDim, headSize,
                cpuModel.configuration() instanceof org.beehive.gpullama3.model.llama.LlamaConfiguration lc
                        ? Float.toString(lc.ropeTheta()) : "n/a");
        System.out.printf("rms scale: cpu=%.9g gpu=%.9g absDiff=%.3g relDiff=%.3g%n",
                cpuScale, gpuTemp[0], Math.abs(cpuScale - gpuTemp[0]),
    ...
    private static void report(String label, float[] ref, float[] got) {
        ...
        System.out.printf("%-38s maxAbs=%.6g meanAbs=%.6g relL2=%.6g maxRel=%.4g bitIdentical=%d/%d%n",
                label, maxAbs, sumAbs / ref.length, Math.sqrt(sumSqDiff / sumSqRef), maxRel,
                exactBits, ref.length);
    }
    ...
    private static List<Integer> promptTokens(Model model) {
        ...
        prompt.addAll(cf.encodeMessage(new ChatFormat.Message(ChatFormat.Role.USER, GoldenCapture.PROMPT)));
