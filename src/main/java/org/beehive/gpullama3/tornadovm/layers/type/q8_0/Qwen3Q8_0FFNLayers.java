...
public class Qwen3Q8_0FFNLayers extends AbstractFFNLayers<Qwen3TornadoWeights, Qwen3Configuration> {
    ...
    private void debugPrintKernelSizes() {
        ...
        System.out.println();
        System.out.println("=== Qwen3 Q8_0 Kernel Size Report ===");
        ...
        System.out.println();
        System.out.println("  Q8_0 block layout (blockSize=32, blockBytes=34: 2-byte FP16 scale + 32 int8):");
        System.out.printf("    attn_qkv input:   blocksPerRow(dim=%d)=%d%n", dim, (dim + 31) / 32);
        System.out.printf("    ffn_gate_up:      blocksPerRow(dim=%d)=%d  outputRows=%d%n", dim, (dim + 31) / 32, hiddenDim);
        System.out.printf("    ffn_down:         blocksPerRow(hiddenDim=%d)=%d  outputRows=%d%n", hiddenDim, (hiddenDim + 31) / 32, dim);
        System.out.printf("    attn_output_proj: blocksPerRow(qDim=%d)=%d  outputRows=%d%n", qDim, (qDim + 31) / 32, dim);
        System.out.println();
        System.out.print("  Valid GEMV LWS (power-of-2, multiple-of-8): ");
        for (int s : new int[]{32, 64, 128, 256}) System.out.printf("%d ", s);
        System.out.println("(all safe for Q8_0 with 4-way unrolling)");
