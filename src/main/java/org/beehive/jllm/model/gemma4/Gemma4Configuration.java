...
public record Gemma4Configuration(
     ...
     *
     * <p>Derived from this model's own attention shape. One workgroup per head is
     * {@code numberOfHeads()} of them — eight here, on a device with more than a hundred
     * multiprocessors — so the window is split until there is enough resident work, then capped so
     * the slices do not become too fine to amortise their own combine: at least 32 positions per
     * slice at the shortest window this model uses.
     *
     ...
     * device this was measured on. The honest form is a device capability; there is no
     * compute-unit count on the backend's {@code Device} contract today, and adding one is a change
     * to a backend interface rather than something a decode kernel should do quietly.
     */
    public int attentionSplits() {
        ...
        int byWindow = Math.max(1, Math.min(slidingWindowSize, contextLength) / minPositionsPerSlice);
